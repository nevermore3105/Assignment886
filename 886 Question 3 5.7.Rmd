...
```{r}
a<-lm(log(crude)~x)
estB<-exp(a$coefficient[1])
estc<-exp(a$coefficient[2])
```
...
iv=c(estB,estc)
iv
f<-function(z){-sum(log((exp(-Ecx*z[1]*z[2]^x)*(Ecx*z[1]*z[2]^x)^dx)/factorial(dx)))}
a1<-nlm(f,iv)
a1
B<-a1$estimate[[1]]
c<-a1$estimate[[2]]
...
grad<-B*c^x
grad
plot(x,crude,ylim=c(0.05,0.15))
lines(x,grad,col="blue")  
```
